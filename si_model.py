...
class Brain:
    ...
    def __init__(self, conn_matrix):
        ...
        # index nodes
        for num_nodes in range(self.num_nodes):
            self.nodes[num_nodes].index = num_nodes

    def choose_infected_node(self):
        '''
        output: index of a randomly selected infected node in the brain
        '''
        if not self.infected_indices: #sanity check whether list is empty
    ...
    def print_configuration(self, snapshot_index):
        """Plot a snapshot of the brain's state.

        Args:
            snapshot_index (int): Index of brains state to plot.

        Returns:
            plot: Matplotlib plot of brain state.
...
class Node:
    ...
    def __init__(self, connect_vector):
        ...
        self.susceptb_neighbors = self.neighbor_indices.copy() # shallow copy
        self.infected_neighbors = set()

    def pick_suscept_neighbor_index(self):
        """Randomly pick a susceptible neighbor according to weights.

        Returns:
            random_suscept_neighbor_index(int): Index of chosen neighbor.

        """
        if not self.susceptb_neighbors:
...
class Simulation:
    """Class for organizing simulation calls.

    Attributes:
        all_simulation_snaps (arr): array of snapshots of brain states.
    ...
    def run_simulation(self, num_steps, brain_index):
        ...
        Args:
            ...
            brain_index (int): index referring to which brain to use.
...
if __name__ == "__main__":
    ...
    FILEPATH = io_utils.get_path(filename=FILENAME)
    conn_matrices = io_utils.get_connectivity_matrices(filepath = FILEPATH)
